...
msg = rospy.wait_for_message('/scan', Laserscan, rosImageVizCallback)
#     [fx'  0  cx' Tx]
# P = [ 0  fy' cy' Ty]
#     [ 0   0   1   0]
...
def main():
    ...
    if not useHSV:
        # Task 1
        # subscribe to image
        rospy.Subscriber('/scan', LaserScan, rosImageVizCallback)
    else:
        if not useDepth:
            # Task 2 Detect object using HSV
            #    Subscribe to RGB images
            rospy.Subscriber('/scan', LaserScan, rosHSVProcessCallBack)
        else:
            ...
            depth_sub = message_filters.Subscriber("/scan", LaserScan)
